...
| `IG_ACCESS_TOKEN` | your long-lived Instagram token (only if using the retired Instagram path) |
...
## Optional: real AI-generated photo backgrounds (Gemini or OpenAI)
...
**This costs real money per image** (see provider tables below — check
the live pricing pages before enabling, this space moves fast: OpenAI
retired its entire DALL-E lineup in May 2026, for example). At 2 posts/day
that's typically a few dollars a month, but it's not free like the
default path, so it's off unless you turn it on.

To enable it, at minimum:

| Name | Value |
|---|---|
| `USE_AI_IMAGES` | `true` |

Then pick a provider with `IMAGE_PROVIDER` (defaults to `gemini` if unset,
so existing setups are unaffected by upgrading):

### Provider: Gemini (default)
Reuses your existing `GEMINI_API_KEY` — no new secret needed.

| Name | Value |
|---|---|
| `IMAGE_PROVIDER` | `gemini` (or just omit — it's the default) |
| `GEMINI_IMAGE_MODEL` | `gemini-2.5-flash-image` (default if unset) |

### Provider: OpenAI
Needs one new secret.
...
| `OPENAI_IMAGE_MODEL` | `gpt-image-1-mini` (default — OpenAI's own recommended choice for high-volume, budget-conscious use) |
| `OPENAI_IMAGE_SIZE` | `1024x1536` (default; portrait, closest to our 4:5 crop) |
| `OPENAI_IMAGE_QUALITY` | `medium` (default; `low`/`medium`/`high` — low is cheapest, good for stretching a small credit balance) |

DALL-E 2 and DALL-E 3 were removed from OpenAI's API in May 2026 — the
current lineup is the GPT Image family. `gpt-image-1-mini` at `low` or
`medium` quality is the cheapest reasonable option and is explicitly what
OpenAI recommends for exactly this kind of daily automated content.

**What happens under the hood**: the pipeline asks the chosen provider's
image model for an editorial-style photo (dark background, warm yellow
accent lighting, cinematic, India-focused visual context, explicitly
instructed to contain zero text/logos/watermarks — AI models reliably
mangle rendered text, so all real text is added afterward with crisp PIL
typography). The overlay is deliberately simpler than the full PIL
templates — just the kicker, headline, CTA, and logo, magazine-cover
style — since a busy bulleted comparison chart doesn't read well on top
of a photo.

**If image generation fails for any reason** — missing key, network error,
safety block, quota exceeded, malformed response, anything, from either
provider — the pipeline automatically falls back to the free PIL renderer
for that post and logs a warning. A bad day for the image API never means
a missed post. This fallback is unconditional.

To go back to the free-only renderer, set `USE_AI_IMAGES` to `false` (or
just delete the variable — that's the default). Switching providers, or
turning this off entirely, is always a Variables/Secrets change only —
never a code change.
